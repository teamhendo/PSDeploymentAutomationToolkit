function Start-GalleryPackageDeploymentProcessing {
[CmdletBinding()]
param (
...
Process {
    do 
    {
        ...
        {
            Write-Log   -Component "DeploymentAutomationToolkit:GalleryPackageDeploymentProcessing" -Type 1 `
            ...
            {
                $deploymentOutcome = Submit-TaniumDeployPackageDeployment -Data $job.$activeRing.deploymentData `
            ...
            if ($deploymentOutcome.deploymentObject.statusLabel -eq 'Active' -or `
                $deploymentOutcome.deploymentObject.statusLabel -eq 'Scheduled' -or `
                $deploymentOutcome.deploymentObject.statusLabel -eq 'Finished') 
            {
        ...
        {
            Write-Log -Component "DeploymentAutomationToolkit:GalleryPackageDeploymentProcessing" `
                      -Type 1 `
                      -LogFile $scriptLogFile `
